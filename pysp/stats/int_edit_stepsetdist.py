...
class IntegerStepBernoulliEditSampler(object):

    def __init__(self, dist: IntegerStepBernoulliEditDistribution, seed: Optional[int] = None):
...
class IntegerStepBernoulliEditAccumulatorFactory(object):
    ...
    def make(self):
        ...
        return IntegerStepBernoulliEditAccumulator(self.num_vals, init_acc=init_acc, keys=self.keys)
...
class IntegerStepBernoulliEditEstimator(ParameterEstimator):
    ...
    def __get_pqk(self,obs_counts,n):
    ...
    def estimate(self, nobs, suff_stat):
        ...
        obs_counts = np.exp(log_pmat[:,1]); n = 1
        arr1 = self.__get_pqk(obs_counts,n)

        obs_counts = np.exp(log_pmat[:,2]); n = 1
        arr2 = self.__get_pqk(obs_counts,n)

        log_pmat[:,2] = np.log(arr2); log_pmat[:,0] = np.log(1-arr2)
        log_pmat[:,1] = np.log(arr1); log_pmat[:,3] = np.log(1-arr1)
